...
New-Item -ItemType Directory -Force -Path 'build/h2-xlsx-custom-out' | Out-Null
java -cp "$h2Jar" org.h2.tools.RunScript -url 'jdbc:h2:file:./build/h2-xlsx-custom-source-db' -user sa -script 'examples/h2-to-h2/sql/source-schema.sql'
java -cp "$h2Jar" org.h2.tools.RunScript -url 'jdbc:h2:file:./build/h2-xlsx-custom-source-db' -user sa -script 'examples/h2-to-h2/sql/source-seed.sql'
...
Write-Host 'Custom H2 -> XLSX done'
